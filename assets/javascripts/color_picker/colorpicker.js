...
Control.ColorPicker.prototype = {
    initialize : function(field, jstoolbar_obj, options) {
        ...
        if (!Control.ColorPicker.CONTROL) {
            ...
            if (!$("colorpicker")) {
                ...
                '<div id="colorpicker-footer"><span id="colorpicker-value">#<input type="text" onclick="this.select()" id="colorpicker-value-input" name="colorpicker-value" value=""></input></span>' +
                '<button id="colorpicker-okbutton">OK</button></div>'
    ...
    close : function(event) {
        ...
        //close when click button or this color panel - to reject called when show
        if(event) {
            var jsToolBarObj = this.jstoolbar;
            jsToolBarObj.encloseSelection(' %','% ',function(str) {
                return "{color:#" +
                document.getElementById(jsToolBarObj.getColorPickerTextId()).value +
                "}" + str;
            });
        }
    },
